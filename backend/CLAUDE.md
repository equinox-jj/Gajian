...
**Current state:** on `feat/auth`, JPA entities exist for every domain module, and the `auth` feature is implemented
end-to-end (`auth/`, `security/`, `config/`, `common/response`, `common/exception`, Flyway
`V1__create_auth_tables.sql`). Every other feature module below the "Planned architecture" heading is still target
design from `docs/`, not code. Only `app_user` and `refresh_token` have migrations — the remaining entities have no
tables yet. Do not assume any package under `com.myproject.gajian` exists until you've checked `src/main/java`.
...
Build this as shared infrastructure so every feature module reuses it rather than rolling its own response shape.
`GlobalExceptionHandler` is the single place that maps exceptions to `error_code` values and the validation-error array;
`security/SecurityErrorHandler` does the same for failures raised inside the security filter chain, which never reach
`@RestControllerAdvice`.

Already built: `common/response/ApiResponse` (success shape) and `common/response/ApiErrorResponse` (error shape — split
in two because success responses always carry `meta` while error responses never do), plus
`common/exception/{GlobalExceptionHandler,ApiException,ErrorCode}`. Still to build, when the first paginated list
endpoint lands: `common/response/PaginatedResponse` and `common/response/PageMeta`.
...
com.myproject.gajian
 ...
 │    └── constant/
 │
 ├── security/                      # Auth/JWT, if not trivial enough for config/
...
- **JWT/auth plumbing lives in `security/`** (`JwtTokenProvider`, `AccessToken`, `CustomUserDetailsService`), separate
  ...
  Security's built-in `BearerTokenAuthenticationFilter` (from `spring-security-oauth2-resource-server`), configured in
  `SecurityConfig` with a `JwtAuthenticationConverter` that maps the `role` claim to a `ROLE_*` authority.
